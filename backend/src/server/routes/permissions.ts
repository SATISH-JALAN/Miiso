...
import { createPermission, updateSecurityProfile, getActivePermission, revokePermission } from "../../db/queries/permissions.js";
    ...
    async (request, reply) => {
      ...
      try {
        ...
        logger.info(`📝 Registering new permission for user ${userAddress}`, {
          sessionSignerAddress, budgetCap, expiryDate: expiryDate.toISOString()
        });

        const permission = await createPermission({
          userAddress, permissionContext, delegationHash, sessionSignerAddress, budgetCap, expiry: expiryDate
        });
        ...
        return reply.status(201).send({
          ...
          permission: {
            ...
            delegationHash: permission.delegationHash, budgetCap: permission.budgetCap, expiry: permission.expiry
          }
        });
      } catch (error: any) {
