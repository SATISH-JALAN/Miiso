...
export function Research() {
  ...
  const getAgentColor = (agent: string) => {
    switch (agent) {
      case 'Relayer': return 'text-[#B8CFA8]';
  ...
  const getAgentIcon = (agent: string) => {
    switch (agent) {
      case 'Relayer': return <DollarSign className="w-4 h-4 text-[#B8CFA8]" />;
  ...
  return (
    <div className="pt-32 pb-24 px-4 sm:px-6 max-w-6xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          <div className="flex items-center gap-2 text-[#B8CFA8] text-xs font-semibold uppercase tracking-widest mb-2">
            <span className="w-2 h-2 rounded-full bg-[#B8CFA8] animate-ping" />
        ...
        <div className="bg-[#101010] p-4 rounded-2xl border border-white/5 flex items-center gap-4">
          <div className="bg-[#B8CFA8]/10 p-3 rounded-xl">
            <DollarSign className="w-6 h-6 text-[#B8CFA8]" />
          </div>
          <div>
      ...
      <div className="bg-[#101010] p-6 rounded-3xl border border-white/5 mb-8 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#B8CFA8]/5 rounded-full blur-3xl -z-10" />
        
        {addressError && (
          <div className="mb-4 p-4 rounded-2xl bg-[#C27A73]/10 border border-red-500/20 flex items-center gap-3">
            <AlertCircle className="w-5 h-5 text-[#C27A73]" />
            <span className="text-[#C27A73] text-sm font-semibold">{addressError}</span>
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="relative flex-1 w-full">
            ...
            <input 
              ...
              disabled={isRunning}
              className="w-full bg-black border border-white/10 rounded-2xl pl-12 pr-4 py-4 text-[#E1E0CC] placeholder-gray-600 focus:outline-none focus:border-[#B8CFA8]/45 transition-colors text-sm"
            />
          </div>
          ...
          <button
            ...
            className={`w-full md:w-auto px-8 py-4 rounded-2xl font-bold transition-all duration-300 flex items-center justify-center gap-2 ${
              isRunning 
                ...
                : 'bg-[#B8CFA8] hover:bg-[#14a361] text-black shadow-lg hover:shadow-[#B8CFA8]/15 hover:scale-[1.02]'
            }`}
          >
            {isRunning ? (
              <>
          ...
          <div className="bg-[#101010] rounded-3xl border border-white/5 p-6 shadow-xl">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/5">
              <h2 className="text-lg font-bold flex items-center gap-2">
                <Bot className="w-5 h-5 text-[#B8CFA8]" />
                Live Agent Task board
              </h2>
              <div className="flex gap-2">
                <button 
                  onClick={() => setActiveTab('interactive')}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${activeTab === 'interactive' ? 'bg-[#B8CFA8]/10 text-[#B8CFA8] border border-[#B8CFA8]/20' : 'text-gray-400 hover:text-white'}`}
                >
                  Interactive Map
                </button>
                <button 
                  onClick={() => setActiveTab('logs')}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${activeTab === 'logs' ? 'bg-[#B8CFA8]/10 text-[#B8CFA8] border border-[#B8CFA8]/20' : 'text-gray-400 hover:text-white'}`}
                >
                  Stdout Logs
                </button>
              </div>
            ...
            {activeTab === 'interactive' ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { id: 'Relayer', title: 'X402 Pay Layer', desc: 'Authorizes micro-payments & limits', color: 'border-[#B8CFA8]/20 bg-[#B8CFA8]/5' },
                  ...
                  return (
                    <div 
                      ...
                      className={`p-5 rounded-2xl border transition-all duration-300 ${agentInfo.color} ${
                        isRunningAgent ? 'ring-2 ring-[#B8CFA8]/50 scale-[1.02]' : ''
                      }`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {isDoneAgent && (
                          <span className="text-[#B8CFA8] text-xs font-semibold bg-[#B8CFA8]/10 px-2 py-0.5 rounded-full">Active</span>
                        )}
                      </div>
              ...
              <div className="bg-black/40 border border-white/10 rounded-2xl p-4 font-mono text-xs h-[320px] overflow-y-auto flex flex-col gap-2">
                {steps.map((s, idx) => (
                  <div key={idx} className={`flex items-start gap-2 ${s.status === 'running' ? 'text-white' : s.status === 'done' ? 'text-gray-500' : 'text-gray-700'}`}>
                    ...
                    <span>{s.message}</span>
                    {s.status === 'running' && <Loader2 className="w-3 h-3 animate-spin text-[#B8CFA8] ml-2 mt-0.5" />}
                  </div>
                ))}
                {currentStepIdx === -1 && (
            ...
            <AnimatePresence mode="wait">
              {currentStepIdx === -1 ? (
                ...
                <motion.div 
                  ...
                  className="flex-1 flex flex-col items-center justify-center text-center p-6"
                >
                  <Loader2 className="w-12 h-12 text-[#B8CFA8] animate-spin mb-4" />
                ...
                >
                  ...
                  <div className={`border p-5 rounded-2xl mb-6 ${reportData?.score && reportData.score < 50 ? 'bg-[#C27A73]/5 border-red-500/20' : 'bg-[#B8CFA8]/5 border-[#B8CFA8]/20'}`}>
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs text-gray-400 font-semibold uppercase">Security Score</span>
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${reportData?.score && reportData.score < 50 ? 'text-[#C27A73] bg-[#C27A73]/10' : 'text-[#B8CFA8] bg-[#B8CFA8]/10'}`}>
                        {reportData?.score && reportData.score < 50 ? 'Vulnerable' : 'Passed'}
                      </span>
                    </div>
                    <div className="flex items-baseline gap-1">
                      <span className={`text-4xl font-extrabold ${reportData?.score && reportData.score < 50 ? 'text-[#C27A73]' : 'text-[#B8CFA8]'}`}>
                        {reportData?.score ?? 92}
                      </span>
                  ...
                  <div className="flex-1 flex flex-col gap-4 text-xs">
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">
                        {reportData?.staticRisks?.length ? (
                          reportData.staticRisks.map((risk, i) => (
                            <span key={i} className="bg-[#C27A73]/10 text-[#C27A73] border border-red-500/20 px-2 py-1 rounded text-[10px]">{risk}</span>
                          ))
                        ) : (
                    ...
                    <div className="pb-3">
                      ...
                      <div className="text-[#B8CFA8] flex items-center gap-1 font-semibold">
                  ...
                  <button 
                    ...
                    className="w-full mt-6 bg-[#B8CFA8] hover:bg-[#14a361] text-black font-bold py-3 rounded-xl transition-all text-xs"
                  >
                    Whitelist Contract
                  </button>
                </motion.div>
